...
Y.EmulateHTML5.Loader = function (config) {
    ...
    var thisOb = this;
    if ( ! (this instanceof Y.EmulateHTML5.Loader) ) {
        thisOb = new Y.EmulateHTML5.Loader(config);
    }
...
Y.EmulateHTML5.Loader.prototype = {
    ...
    load: function (components) {
        var yuiLoaderArguments = [], name; // Declare variables
        
        // Get components as the default arguments merged with new ones
        components = Y.merge(this.arg.components,this._setupComponentsOb(components));
    ...
    _initializeComponents: function (Y) {
        // Define variables
        var namespace = this.arg.yuiNamespace, components = this.arg.components, name, throwawayObject;
            ...
            for ( name in components ) {
                if ( components.hasOwnProperty(name) ) {
                    ...
                    if( Y.Lang.isFunction(Y[namespace][name])) {
                        ...
                        // Load it
                        throwawayObject = new Y[namespace][name](this.arg.components[name]);
...
/* Close YUI closure */
    ...
    groups: {
      gallery: {
        base    : '../../../yui3-gallery/build/', // Adjust as necessary.
        modules: {
